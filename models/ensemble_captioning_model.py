...
class EsembleCaptioningModel(CaptioningModel):
    """
    In an Ensemble, the output are averaged and the Encoder part is separated
    ...
    def __init__(self, models_list, rank):
        ...
        # this is nothing but a trick so it can be encapsulated inside a DDP object bypassing
        # the assertion of requiring trainable parameters
        self.dummy_linear = nn.Linear(1, 1)

        # setto in evaluation mode i modelli passati in input
    ...
    def forward_dec(self, cross_input_list, enc_input_num_pads, dec_input, dec_input_num_pads, apply_log_softmax=False):
        ...
        y_outputs = []
        for i in range(self.num_models):
            y_outputs.append(F.softmax(self.models_list[i].forward_dec(
    ...
    # is a bit similar to the Single model case
