...
class CaptioningModel(nn.Module):
    ...
    def get_batch_multiple_sampled_prediction(self, enc_input, enc_input_num_pads, num_outputs,
        ...
        # res_predicted_sentece: list of list of ints [bs, num_outpus, description token indexes]
        # predicted_caption_prob: list of list of floats [bs, num_outputs, logprobs for each token]
    ...
    def beam_search(self, enc_input, enc_input_num_pads, sos_idx, eos_idx,
        ...
        for time_step in range(2, max_seq_len):
            ...
            # However, if we pad with -999 its candidates logprobabilities, also the sequence containing EOS would be
            ...
            lastword_top_beam_size_classes = which_sequence_top_beam_size_word_classes.gather(dim=-1,
                                                                                              index=which_word)  # [bs, beam_size, 1]
    ...
    def write_model_summary(self, writer):
        # recursively print all parameters
        def recursively_add_histogram_weights(tree_node, description='train_weights'):
            if len(tree_node._modules) != 0:  # just a container
                for key in tree_node._modules.keys():
                    recursively_add_histogram_weights(tree_node._modules[key], description=description + '/' + key)
            elif 'weight' in tree_node._parameters.keys():
                writer.add_histogram(description + '/',
                                     tree_node.weight, self.trained_steps)
        recursively_add_histogram_weights(self, 'train_weights')

        # recursively print all parameters
        def recursively_add_histogram_grad(tree_node, description='train_grad'):
            if len(tree_node._modules) != 0:  # just a container
                for key in tree_node._modules.keys():
                    recursively_add_histogram_grad(tree_node._modules[key], description=description + '/' + key)
            elif 'weight' in tree_node._parameters.keys():
                writer.add_histogram(description + '/',
                                     tree_node.weight._grad, self.trained_steps)

        recursively_add_histogram_grad(self, 'train_grad')
